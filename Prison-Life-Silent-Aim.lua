...
local whitelistMode = "both"
local whitelistPlayerList = {}
local whitelistConnections = {}
...
local function isWhitelisted(player)
	if not whitelistEnabled or not player then return false end
	if whitelistMode == "userId" or whitelistMode == "both" then
		if whitelist[player.UserId] then return true end
	end
	if whitelistMode == "username" or whitelistMode == "both" then
		local normalized = normalizeUsername(player.Name)
		if whitelist[normalized] then return true end
		local displayNormalized = normalizeUsername(player.DisplayName)
		if whitelist[displayNormalized] then return true end
	end
	return false
end

local function addToWhitelist(identifier, playerInfo)
	local id = tonumber(identifier)
	if id then
		whitelist[id] = playerInfo or true
		return "userId", id
	end
	local normalized = normalizeUsername(identifier)
	if normalized ~= "" then
		whitelist[normalized] = playerInfo or true
		return "username", normalized
	end
	return nil, nil
end

local function removeFromWhitelist(identifier)
	local id = tonumber(identifier)
	if id and whitelist[id] then whitelist[id] = nil return true end
	local normalized = normalizeUsername(identifier)
	if normalized ~= "" and whitelist[normalized] then whitelist[normalized] = nil return true end
	return false
end

local function clearWhitelist() whitelist = {} end

local function getWhitelistList()
	local list = {}
	for key, info in pairs(whitelist) do
		local display = tostring(key)
		if typeof(info) == "table" and info.name then
			display = info.name .. " (" .. key .. ")"
		end
...
local function getWhitelistSaveData()
	local list = {}
	for key, info in pairs(whitelist) do
		table.insert(list, {key = tostring(key), name = (typeof(info) == "table" and info.name) or nil, displayName = (typeof(info) == "table" and info.displayName) or nil})
	end
	return list
end
...
local function updatePlayerList()
	...
	for _, player in ipairs(Players:GetPlayers()) do
		if player ~= LocalPlayer then table.insert(whitelistPlayerList, player) end
	end
	table.sort(whitelistPlayerList, function(a, b) return a.Name:lower() < b.Name:lower() end)
end

local function getPlayerListOptions()
	local options = {}
	for _, player in ipairs(whitelistPlayerList) do
		local marker = isWhitelisted(player) and " [✓]" or ""
		table.insert(options, player.Name .. " (" .. player.DisplayName .. ")" .. marker)
	end
	return options
end

local function getPlayerFromOption(optionText)
	local name = tostring(optionText):match("^([^%(]+)")
	if name then
		name = name:gsub("%s+$", "")
		for _, player in ipairs(whitelistPlayerList) do
...
local function setupPlayerTracking()
	for _, conn in ipairs(whitelistConnections) do conn:Disconnect() end
	whitelistConnections = {}
	updatePlayerList()
	table.insert(whitelistConnections, Players.PlayerAdded:Connect(function() task.defer(updatePlayerList) end))
	table.insert(whitelistConnections, Players.PlayerRemoving:Connect(function() task.defer(updatePlayerList) end))
end
...
local function saveWhitelist()
	...
	local saveData = {enabled = whitelistEnabled, mode = whitelistMode, list = getWhitelistSaveData()}
	writefile(configFolder .. "/whitelist.json", HttpService:JSONEncode(saveData))
end

local function loadWhitelist()
	if not isfile then return end
	local path = configFolder .. "/whitelist.json"
	if not isfile(path) then return end
	local success, data = pcall(function() return HttpService:JSONDecode(readfile(path)) end)
	if not success or not data then return end
	whitelistEnabled = data.enabled or false
	whitelistMode = data.mode or "userId"
	clearWhitelist()
	for _, entry in ipairs(data.list or {}) do
		local key = entry.key
		local playerInfo = entry.name and {name = entry.name, displayName = entry.displayName} or nil
		local id = tonumber(key)
		if id then whitelist[id] = playerInfo or true
		else whitelist[key:lower()] = playerInfo or true end
	end
end

-- ==================== WHITELIST UI FUNCTIONS (use notify, not Window:Notify) ====================
local uiElements = {} -- single table, no pre-declared nils
local _selectedWhitelistPlayer = nil

local function refreshWhitelistPlayerList()
	local dropdown = uiElements.WhitelistPlayerDropdown
	if not dropdown then return end
	updatePlayerList()
	local options = getPlayerListOptions()
	pcall(function() dropdown:ClearOptions() end)
	pcall(function() dropdown:InsertOptions(options) end)
end

local function addSelectedPlayerToWhitelist(optionText)
	local player = getPlayerFromOption(optionText)
	if not player then
		notify("Whitelist", "Could not find player!", 3)
		return false
	end
	local info = {name = player.Name, displayName = player.DisplayName}
	addToWhitelist(tostring(player.UserId), info)
	addToWhitelist(player.Name, info)
	saveWhitelist()
	notify("Whitelist", "Added " .. player.Name .. " to whitelist", 3)
	task.defer(refreshWhitelistPlayerList)
	return true
end

local function removeSelectedPlayerFromWhitelist(optionText)
	local player = getPlayerFromOption(optionText)
	if not player then
		notify("Whitelist", "Could not find player!", 3)
		return false
	end
	removeFromWhitelist(tostring(player.UserId))
	removeFromWhitelist(player.Name)
	saveWhitelist()
	notify("Whitelist", "Removed " .. player.Name .. " from whitelist", 3)
	task.defer(refreshWhitelistPlayerList)
	return true
end

setupPlayerTracking()
task.defer(loadWhitelist)
...
local function startLinger(target, targetPos, gun)
	if not target or not targetPos or not gun then return end
	lingerState.active = true
	lingerState.lastValidTarget = target
	lingerState.lastValidTargetPos = targetPos
	lingerState.lastValidGun = gun
	lingerState.endTime = os.clock() + rng:NextNumber(0.05, 1.0)
end

local function stopLinger()
	lingerState.active = false
	lingerState.lastValidTarget = nil
	lingerState.lastValidTargetPos = nil
	lingerState.lastValidGun = nil
	lingerState.endTime = 0
end

local function isLingering()
	return lingerState.active and os.clock() < lingerState.endTime
end
...
-- ==================== STATE VARIABLES ====================
local state.currentGun = nil
local state.rng = Random.new()
local state.lastShotTime = 0
local state.lastShotResult = false
local state.shotCooldown = 1 / 30
local state.currentTarget = nil
local state.targetSwitchTime = 0
local state.currentStickiness = 0
local state.randomPartCache = {}
local state.lastTouchAimPos = nil
local state.activeTouch = nil
local state.mobileGuiButton = nil
local state.mobileGuiDragInput = nil
local state.lastAutoShoot = 0
local state.cachedBulletsLabel = nil
local state.targetAcquiredTime = 0
local state.lastAutoTarget = nil
local state.lastReloadRequest = 0
local state.playerSettings = ReplicatedStorage:FindFirstChild("PlayerSettings")
local state.mobileCursorOffset = 0
local state.isInsideDynThumbFrame = nil
local state.giverPressedRemote = ReplicatedStorage:FindFirstChild("Remotes") and ReplicatedStorage.Remotes:FindFirstChild("GiverPressed")
local state.trackedGrabbables = {}
local state.firstSeenGrabbables = {}
local state.lastAutoGrab = 0

do
	local state.sharedModules = ReplicatedStorage:FindFirstChild("SharedModules")
	local state.dynThumbModule = sharedModules and sharedModules:FindFirstChild("isInsideDynThumbFrame")
...
-- Pre-declare all UI elements to prevent nil errors
uiElements = {
    SilentAimToggle = nil,
    TeamCheckToggle = nil,
    WallCheckToggle = nil,
    DeathCheckToggle = nil,
    VehicleCheckToggle = nil,
    HostileCheckToggle = nil,
    TrespassCheckToggle = nil,
    CriminalsSkipInmatesToggle = nil,
    InmatesSkipCriminalsToggle = nil,
    FFCheckToggle = nil,
    ShieldBreakerToggle = nil,
    ShieldFrontAngleSlider = nil,
    ShieldRandomHeadToggle = nil,
    ShieldHeadChanceSlider = nil,
    TaserBypassHostileToggle = nil,
    TaserBypassTrespassToggle = nil,
    TaserAlwaysHitToggle = nil,
    HitIfPlayerStillToggle = nil,
    StillThresholdSlider = nil,
    HitChanceSlider = nil,
    HitChanceAutoOnlyToggle = nil,
    DistanceBasedHitChanceToggle = nil,
    DistanceHitchancePointDropdown = nil,
    AimMaxDistSlider = nil,
    MissSpreadSlider = nil,
    ShotgunNaturalSpreadToggle = nil,
    ShotgunGameHandledToggle = nil,
    PrioritizeClosestToggle = nil,
    PrioritizeCriminalsToggle = nil,
    TargetStickinessToggle = nil,
    StickinessDurationSlider = nil,
    TargetStickinessRandomToggle = nil,
    TargetStickinessMinSlider = nil,
    TargetStickinessMaxSlider = nil,
    FovSlider = nil,
    ShowFovToggle = nil,
    StaticFovToggle = nil,
    ShowTargetLineToggle = nil,
    AimPartDropdown = nil,
    RandomPartsToggle = nil,
    EspToggle = nil,
    EspTeamCheckToggle = nil,
    ShowTeamToggle = nil,
    EspMaxDistanceSlider = nil,
    ShowDistanceToggle = nil,
    UseTeamColorsToggle = nil,
    EspColorPicker = nil,
    GuardsColorPicker = nil,
    InmatesColorPicker = nil,
    CriminalsColorPicker = nil,
    TeamColorPicker = nil,
    C4EspToggle = nil,
    C4EspMaxDistanceSlider = nil,
    C4ShowDistanceToggle = nil,
    C4EspColorPicker = nil,
    AutoshootToggle = nil,
    AutoshootWeaponDropdown = nil,
    AutoshootDelaySlider = nil,
    AutoshootStartDelaySlider = nil,
    AutoGrabToggle = nil,
    AutoGrabDistanceSlider = nil,
    AutoGrabDelaySlider = nil,
    AutoGrabKeycardToggle = nil,
    AutoGrabM9Toggle = nil,
    WhitelistEnabledToggle = nil,
    WhitelistModeDropdown = nil,
    WhitelistPlayerDropdown = nil,
    WhitelistInput = nil,
    SelectedDistancePointDistanceSlider = nil,
    SelectedDistancePointHitchanceSlider = nil,
...
local function syncDistanceHitchanceEditor()
	...
	if selectedPoint then
		if uiElements.SelectedDistancePointDistanceSlider then
			uiElements.SelectedDistancePointDistanceSlider:SetValue(selectedPoint.distance)
		end
		if uiElements.SelectedDistancePointHitchanceSlider then
			uiElements.SelectedDistancePointHitchanceSlider:SetValue(selectedPoint.chance)
		end
	end
...
local function loadLegitDistanceHitchancePreset()
	...
	cfg.distancehitchancepoints = {
		{distance = 10,  chance = 85},
		{distance = 20,  chance = 78},
		{distance = 35,  chance = 68},
		{distance = 50,  chance = 58},
		{distance = 75,  chance = 48},
		{distance = 100, chance = 38},
		{distance = 200, chance = 22},
		{distance = 250, chance = 15},
		{distance = 300, chance = 10},
		{distance = 400, chance = 5}
	}
...
local function shouldShowEsp(player)
	...
	-- WHITELIST CHECK: Don't ESP whitelisted players
	if whitelistEnabled and isWhitelisted(player) then
		return false
	end
...
local function wallBetween(startPos, endPos, targetChar)
	...
	-- Build filter list (ignore self and target)
	local filter = {myChar}
	if targetChar then
		table.insert(filter, targetChar)
		for _, child in ipairs(targetChar:GetDescendants()) do
			if child:IsA("Tool") or child:IsA("Accessory") or child:IsA("Hat") then
	...
	for i = 1, 15 do
		...
		-- Check if it's a fence/bar/gate - these SHOULD block in RCL
		local isFence = nameLower:find("fence") or
		                nameLower:find("bars") or
		                nameLower:find("gate") or
		                nameLower:find("chain") or
		                nameLower:find("barrier") or
		                nameLower:find(" railing") or
		                (material == Enum.Material.Metal and hit.Transparency > 0.2 and hit.Transparency < 0.9)
		
		-- Check if it's glass/window - these should NOT block
		local isGlass = material == Enum.Material.Glass or 
		                material == Enum.Material.Ice or
		                material == Enum.Material.ForceField or
		                nameLower:find("glass") or
		                nameLower:find("window") or
		                hit.Transparency >= 0.9
		
		-- Check if it's thin decorative - these should NOT block
		local isThin = hit.Size and math.min(hit.Size.X, hit.Size.Y, hit.Size.Z) < 0.3
		
		-- FENCES BLOCK - don't let shots through them
		if isFence then
			-- Do a secondary check to make sure it's actually covering the path
			local fenceThickness = 0
			local backResult = workspace:Raycast(result.Position + unit * 0.05, -unit * 1, wallParams)
			if backResult and backResult.Instance == hit then
				fenceThickness = (result.Position - backResult.Position).Magnitude
			end
			-- If it's a real fence structure (has some thickness or is metal)
			if fenceThickness > 0.05 or material == Enum.Material.Metal then
				return true -- BLOCKED by fence
			end
		end
		
						-- Glass doesn't block - shoot through it
		if isGlass then
			remaining = remaining - hitDist - 0.05
			if remaining <= 0 then return false end
			currentStart = result.Position + unit * 0.05
			continue
		end
		
		-- Thin decorative parts don't block
		if isThin then
			remaining = remaining - hitDist - 0.05
			if remaining <= 0 then return false end
			currentStart = result.Position + unit * 0.05
		...
		-- Check if it's actually a solid wall
		local isSolidWall = hit.CanCollide and 
		                    hit.Transparency < 0.5 and
		                    not hit:IsA("TrussPart") and
		                    not (hit.Size and math.max(hit.Size.X, hit.Size.Y, hit.Size.Z) < 0.3)
		
		if isSolidWall then
			solidHitCount = solidHitCount + 1
			
			if solidHitCount >= 2 then
				return true
			end
			
			-- Check thickness
			local backResult = workspace:Raycast(
				result.Position + unit * 0.1, 
				-unit * 2, 
				wallParams
			)
			...
			local thickness = 2
			if backResult and backResult.Instance == hit then
				thickness = (result.Position - backResult.Position).Magnitude
			end
			...
			-- Offset checks for solid coverage
			local blockedOffsets = 0
			
			for offsetI = 1, 3 do
				local angle = (offsetI / 3) * math.pi * 2
				...
				local perpOffset = right * math.cos(angle) * 0.5 + up * math.sin(angle) * 0.5
				local offsetStart = startPos + perpOffset
				local offsetEnd = endPos + perpOffset
				local offsetDir = (offsetEnd - offsetStart).Unit
				local offsetDist = (offsetEnd - offsetStart).Magnitude
				
				local offsetResult = workspace:Raycast(offsetStart, offsetDir * offsetDist, wallParams)
				if offsetResult and offsetResult.Instance == hit then
					blockedOffsets = blockedOffsets + 1
				end
			end
			
			if blockedOffsets >= 2 then
				return true
			end
		end
		
		-- Continue past this object
		remaining = remaining - hitDist - 0.05
		if remaining <= 0 then return false end
		currentStart = result.Position + unit * 0.05
	end
...
local function quickCheck(player)
	...
	-- WHITELIST CHECK: If whitelist is enabled and player is whitelisted, skip them
	if whitelistEnabled and isWhitelisted(player) then
		return false
	end
...
local function getClosest(fovRadius)
    ...
    if cfg.targetstickiness and currentTarget and (now - targetSwitchTime) < currentStickiness then
        if currentTarget.Character and fullCheck(currentTarget) then
...
local function autoShoot()
    ...
    -- Check if gun is still equipped
    if not gun.Parent or gun.Parent ~= LocalPlayer.Character then 
        stopLinger()
        return 
    end
    ...
    local now = os.clock()
    local myChar = LocalPlayer.Character
    if not isValidCharacter(myChar) then 
        stopLinger()
        return 
    end
    ...
    local targetIsValid = target and fullCheck(target)
    local targetPart = target and getTargetPart(target.Character)

    -- Linger logic
    if targetIsValid and targetPart then
        if target ~= lingerState.lastValidTarget then
            startLinger(target, targetPart.Position, gun)
        else
            lingerState.lastValidTargetPos = targetPart.Position
            lingerState.lastValidGun = gun
            lingerState.endTime = now + rng:NextNumber(0.05, 1.0)
        end
        
        lastAutoTarget = target
        targetAcquiredTime = now
        
    elseif isLingering() and lingerState.lastValidTargetPos and lingerState.lastValidGun == gun then
        targetPos = lingerState.lastValidTargetPos
        targetPart = {
            Position = lingerState.lastValidTargetPos,
            Name = cfg.aimpart
        }
        target = lingerState.lastValidTarget
    else
        stopLinger()
        lastAutoTarget = nil
        return 
    ...
    if lingerState.active and now >= lingerState.endTime then
        stopLinger()
    ...
    if targetIsValid and target.Character then
    ...
    if isLingering() then
        shouldHit = true
    elseif cfg.taseralwayshit and isTaser then
        shouldHit = true
    elseif cfg.ifplayerstill and target and isStanding(target) then
        shouldHit = true
...
RunService.Heartbeat:Connect(function()
	local now = os.clock()
	state.currentGun = getGun()
	if state.currentGun ~= state.lastGun then
		resetAimState()
		state.lastGun = state.currentGun
	end
	updateAutoGrab(now)
	autoShoot()
end)
...
local function findCastRay()
    -- Try filtergc first (old executors)
    if filtergc then
        local f = filtergc("function", {Name = "castRay"}, true)
        if f then return f end
    end
    
    -- Alternative: scan for the function by name/signature (more universal)
    for _, v in ipairs(getgc(true)) do
        if typeof(v) == "function" then
            local success, name = pcall(debug.getinfo, v, "n")
            if success and name and name.name == "castRay" then
                return v
            end
        end
    end
    
    -- Last resort: common names in Prison Life
    local remotes = ReplicatedStorage:FindFirstChild("GunRemotes")
    if remotes then
        -- Sometimes it's in modules
        for _, mod in ipairs(getgc(true)) do
            if typeof(mod) == "function" and getfenv(mod).script then
                local scriptName = getfenv(mod).script.Name
                if scriptName:find("Gun") or scriptName:find("Bullet") then
...
local function setupHook()
    local castRayFunc = findCastRay()
    if not castRayFunc then
        warn("Could not find castRay function!")
        return false
    end
    ...
    origCastRay = hookfunction(castRayFunc, noUpvals(function(startPos, targetPos, ...)
        ...
        if closest and closest.Character then
            -- ... (keep your existing hook logic here)
            local gun = currentGun
            if not gun then
                return origCastRay(startPos, targetPos, ...)
            end
...
if not setupHook() then
    task.spawn(function()
        local attempts = 0
        while not hooked and attempts < 20 do
            task.wait(0.5)
            attempts += 1
            if setupHook() then
                hooked = true
                print("Silent Aim hook successful!")
                break
            end
        end
        if not hooked then
            warn("Failed to hook castRay after 20 attempts.")
        end
    end)
else
    hooked = true
...
-- ==================== UI CREATION (wrapped in function to limit scope) ====================
local function createUI()
	local MacLib = nil
	local success, err = pcall(function()
		MacLib = loadstring(game:HttpGet("https://raw.githubusercontent.com/biggaboy212/Maclib/main/maclib.txt"))()
	end)

	if not success or not MacLib then
		warn("Failed to load MacLib UI: " .. tostring(err))
		notify("Silent Aim", "UI failed to load. Script partially running.", 5)
		return
	end
	...
	local function getPreferredWindowSize()
		...
		if UserInputService.TouchEnabled then
			...
			return UDim2.fromOffset(
				math.floor(math.clamp(viewport.X * 0.9, minWidth, maxWidth)),
				math.floor(math.clamp(viewport.Y * 0.8, minHeight, maxHeight))
			)
		end
	...
	local function bindWindowResizeToCamera()
		...
		currentViewportConn = camera:GetPropertyChangedSignal("ViewportSize"):Connect(function()
			if UserInputService.TouchEnabled then
				...
				if state.clampMobileButtonPosition then state.clampMobileButtonPosition() end
			end
		end)
	end
	...
	workspace:GetPropertyChangedSignal("CurrentCamera"):Connect(function()
		...
		if UserInputService.TouchEnabled then task.defer(applyPreferredWindowSize) end
	end)
	bindWindowResizeToCamera()

	-- ==================== MOBILE BUTTON ====================
	local function getMobileButtonParent() return (gethui and gethui()) or CoreGui end
	...
	local function getClampedButtonCenter(rawCenter, buttonSize)
		...
		return Vector2.new(math.clamp(rawCenter.X, minX, maxX), math.clamp(rawCenter.Y, minY, maxY))
	end

	local function getDefaultMobileButtonCenter(buttonSize)
		local viewport = workspace.CurrentCamera and workspace.CurrentCamera.ViewportSize or Vector2.new(1920, 1080)
		local topOffset = math.max(118, viewport.Y * 0.12)
		return getClampedButtonCenter(Vector2.new(buttonSize.X * 0.5 + 28, topOffset + buttonSize.Y * 0.5), buttonSize)
	end

	state.clampMobileButtonPosition = function()
		if not state.mobileGuiButton or not state.mobileGuiButton.Parent then return end
		local currentCenter = Vector2.new(
			state.mobileGuiButton.AbsolutePosition.X + state.mobileGuiButton.AbsoluteSize.X * 0.5,
			state.mobileGuiButton.AbsolutePosition.Y + state.mobileGuiButton.AbsoluteSize.Y * 0.5
		)
		local clampedCenter = getClampedButtonCenter(currentCenter, state.mobileGuiButton.AbsoluteSize)
		state.mobileGuiButton.Position = UDim2.fromOffset(clampedCenter.X, clampedCenter.Y)
	end

	state.resolveMacLibWindowGui = function()
		if not Window then return nil end
		local title = Window.Settings and Window.Settings.Title or "Prison Life Silent Aim"
		local guiParent = (gethui and gethui()) or CoreGui
		for _, parent in ipairs({guiParent, CoreGui}) do
			for _, desc in ipairs(parent:GetDescendants()) do
				if desc:IsA("TextLabel") and desc.Text == title then
					return desc:FindFirstAncestorOfClass("ScreenGui")
				end
			end
		end
		return nil
	...
	state.refreshMobileButtonVisibility = function()
		if not state.mobileGuiButton then return end
		state.mobileGuiButton.Visible = true
		state.mobileGuiButton.Active = true
	end

	state.syncMobileButtonLayer = function()
		if not state.mobileGuiButton then return end
		local screen = state.mobileGuiButton:FindFirstAncestorOfClass("ScreenGui")
		if not screen then return end
		local windowGui = state.resolveMacLibWindowGui and state.resolveMacLibWindowGui() or nil
		local targetParent = getMobileButtonParent()
		if windowGui and windowGui.Parent then targetParent = windowGui.Parent end
		screen.ScreenInsets = Enum.ScreenInsets.None
		screen.DisplayOrder = windowGui and windowGui.DisplayOrder or 2147483647
		if screen.Parent ~= targetParent then screen.Parent = targetParent end
		screen.Parent = nil
		screen.Parent = targetParent
	end

	local function toggleMacLibWindow()
		local windowGui = state.resolveMacLibWindowGui()
		...
		state.refreshMobileButtonVisibility()
	end
	...
	local function createMobileWindowButton()
		...
		button.InputBegan:Connect(function(input)
			...
			dragInput = input
			state.mobileGuiDragInput = input
			dragStart = Vector2.new(input.Position.X, input.Position.Y)
			startCenter = Vector2.new(button.AbsolutePosition.X + button.AbsoluteSize.X * 0.5, button.AbsolutePosition.Y + button.AbsoluteSize.Y * 0.5)
			state.activeTouch = nil
			state.lastTouchAimPos = nil
			moved = false
		end)
		...
		UserInputService.InputChanged:Connect(function(input)
			if input == dragInput and dragStart and startCenter then
				...
				if not moved then
					if delta.Magnitude < moveThreshold then return end
					moved = true
				end
		...
		UserInputService.InputEnded:Connect(function(input)
			...
			state.mobileGuiDragInput = nil
			dragInput = nil
			dragStart = nil
			startCenter = nil
			moved = false
			if wasMoved then state.clampMobileButtonPosition() else toggleMacLibWindow() end
		end)

		task.defer(state.clampMobileButtonPosition)
		task.defer(state.syncMobileButtonLayer)
		task.defer(state.refreshMobileButtonVisibility)
	end
...
local function refreshUI()
	if uiElements.SilentAimToggle then uiElements.SilentAimToggle:SetValue(cfg.enabled) end
	if uiElements.TeamCheckToggle then uiElements.TeamCheckToggle:SetValue(cfg.teamcheck) end
	if uiElements.WallCheckToggle then uiElements.WallCheckToggle:SetValue(cfg.wallcheck) end
	if uiElements.DeathCheckToggle then uiElements.DeathCheckToggle:SetValue(cfg.deathcheck) end
	if uiElements.VehicleCheckToggle then uiElements.VehicleCheckToggle:SetValue(cfg.vehiclecheck) end
	if uiElements.HostileCheckToggle then uiElements.HostileCheckToggle:SetValue(cfg.hostilecheck) end
	if uiElements.TrespassCheckToggle then uiElements.TrespassCheckToggle:SetValue(cfg.trespasscheck) end
	if uiElements.CriminalsSkipInmatesToggle then uiElements.CriminalsSkipInmatesToggle:SetValue(cfg.criminalsnoinnmates) end
	if uiElements.InmatesSkipCriminalsToggle then uiElements.InmatesSkipCriminalsToggle:SetValue(cfg.inmatesnocriminals) end
	if uiElements.FFCheckToggle then uiElements.FFCheckToggle:SetValue(cfg.ffcheck) end
	if uiElements.ShieldBreakerToggle then uiElements.ShieldBreakerToggle:SetValue(cfg.shieldbreaker) end
	if uiElements.ShieldFrontAngleSlider then uiElements.ShieldFrontAngleSlider:SetValue(cfg.shieldfrontangle) end
	if uiElements.ShieldRandomHeadToggle then uiElements.ShieldRandomHeadToggle:SetValue(cfg.shieldrandomhead) end
	if uiElements.ShieldHeadChanceSlider then uiElements.ShieldHeadChanceSlider:SetValue(cfg.shieldheadchance) end
	if uiElements.TaserBypassHostileToggle then uiElements.TaserBypassHostileToggle:SetValue(cfg.taserbypasshostile) end
	if uiElements.TaserBypassTrespassToggle then uiElements.TaserBypassTrespassToggle:SetValue(cfg.taserbypasstrespass) end
	if uiElements.TaserAlwaysHitToggle then uiElements.TaserAlwaysHitToggle:SetValue(cfg.taseralwayshit) end
	if uiElements.HitIfPlayerStillToggle then uiElements.HitIfPlayerStillToggle:SetValue(cfg.ifplayerstill) end
	if uiElements.StillThresholdSlider then uiElements.StillThresholdSlider:SetValue(cfg.stillthreshold) end
	if uiElements.HitChanceSlider then uiElements.HitChanceSlider:SetValue(cfg.hitchance) end
	if uiElements.HitChanceAutoOnlyToggle then uiElements.HitChanceAutoOnlyToggle:SetValue(cfg.hitchanceAutoOnly) end
	if uiElements.DistanceBasedHitChanceToggle then uiElements.DistanceBasedHitChanceToggle:SetValue(cfg.distancebasedhitchance) end
	if uiElements.AimMaxDistSlider then uiElements.AimMaxDistSlider:SetValue(cfg.aimmaxdist) end
	if uiElements.MissSpreadSlider then uiElements.MissSpreadSlider:SetValue(cfg.missspread) end
	if uiElements.ShotgunNaturalSpreadToggle then uiElements.ShotgunNaturalSpreadToggle:SetValue(cfg.shotgunnaturalspread) end
	if uiElements.ShotgunGameHandledToggle then uiElements.ShotgunGameHandledToggle:SetValue(cfg.shotgungamehandled) end
	if uiElements.PrioritizeClosestToggle then uiElements.PrioritizeClosestToggle:SetValue(cfg.prioritizeclosest) end
	if uiElements.PrioritizeCriminalsToggle then uiElements.PrioritizeCriminalsToggle:SetValue(cfg.prioritizecriminals) end
	if uiElements.TargetStickinessToggle then uiElements.TargetStickinessToggle:SetValue(cfg.targetstickiness) end
	if uiElements.StickinessDurationSlider then uiElements.StickinessDurationSlider:SetValue(cfg.targetstickinessduration) end
	if uiElements.TargetStickinessRandomToggle then uiElements.TargetStickinessRandomToggle:SetValue(cfg.targetstickinessrandom) end
	if uiElements.TargetStickinessMinSlider then uiElements.TargetStickinessMinSlider:SetValue(cfg.targetstickinessmin) end
	if uiElements.TargetStickinessMaxSlider then uiElements.TargetStickinessMaxSlider:SetValue(cfg.targetstickinessmax) end
	if uiElements.FovSlider then uiElements.FovSlider:SetValue(cfg.fov) end
	if uiElements.ShowFovToggle then uiElements.ShowFovToggle:SetValue(cfg.showfov) end
	if uiElements.StaticFovToggle then uiElements.StaticFovToggle:SetValue(cfg.staticfov) end
	if uiElements.ShowTargetLineToggle then uiElements.ShowTargetLineToggle:SetValue(cfg.showtargetline) end
	if uiElements.AimPartDropdown then uiElements.AimPartDropdown:SetValue(cfg.aimpart) end
	if uiElements.RandomPartsToggle then uiElements.RandomPartsToggle:SetValue(cfg.randomparts) end
	if uiElements.EspToggle then uiElements.EspToggle:SetValue(cfg.esp) end
	if uiElements.EspTeamCheckToggle then uiElements.EspTeamCheckToggle:SetValue(cfg.espteamcheck) end
	if uiElements.ShowTeamToggle then uiElements.ShowTeamToggle:SetValue(cfg.espshowteam) end
	if uiElements.EspMaxDistanceSlider then uiElements.EspMaxDistanceSlider:SetValue(cfg.espmaxdist) end
	if uiElements.ShowDistanceToggle then uiElements.ShowDistanceToggle:SetValue(cfg.espshowdist) end
	if uiElements.UseTeamColorsToggle then uiElements.UseTeamColorsToggle:SetValue(cfg.espuseteamcolors) end
	if uiElements.EspColorPicker then uiElements.EspColorPicker:SetValue(cfg.espcolor) end
	if uiElements.GuardsColorPicker then uiElements.GuardsColorPicker:SetValue(cfg.espguards) end
	if uiElements.InmatesColorPicker then uiElements.InmatesColorPicker:SetValue(cfg.espinmates) end
	if uiElements.CriminalsColorPicker then uiElements.CriminalsColorPicker:SetValue(cfg.espcriminals) end
	if uiElements.TeamColorPicker then uiElements.TeamColorPicker:SetValue(cfg.espteam) end
	if uiElements.C4EspToggle then uiElements.C4EspToggle:SetValue(cfg.c4esp) end
	if uiElements.C4EspMaxDistanceSlider then uiElements.C4EspMaxDistanceSlider:SetValue(cfg.c4espmaxdist) end
	if uiElements.C4ShowDistanceToggle then uiElements.C4ShowDistanceToggle:SetValue(cfg.c4espshowdist) end
	if uiElements.C4EspColorPicker then uiElements.C4EspColorPicker:SetValue(cfg.c4espcolor) end
	if uiElements.AutoshootToggle then uiElements.AutoshootToggle:SetValue(cfg.autoshoot) end
	if uiElements.AutoshootWeaponDropdown then uiElements.AutoshootWeaponDropdown:SetValue(cfg.autoshootweapon) end
	if uiElements.AutoshootDelaySlider then uiElements.AutoshootDelaySlider:SetValue(cfg.autoshootdelay) end
	if uiElements.AutoshootStartDelaySlider then uiElements.AutoshootStartDelaySlider:SetValue(cfg.autoshootstartdelay) end
	if uiElements.AutoGrabToggle then uiElements.AutoGrabToggle:SetValue(cfg.autograb) end
	if uiElements.AutoGrabDistanceSlider then uiElements.AutoGrabDistanceSlider:SetValue(cfg.autograbdistance) end
	if uiElements.AutoGrabDelaySlider then uiElements.AutoGrabDelaySlider:SetValue(cfg.autograbdelay) end
	if uiElements.AutoGrabKeycardToggle then uiElements.AutoGrabKeycardToggle:SetValue(cfg.autograbkeycard) end
	if uiElements.AutoGrabM9Toggle then uiElements.AutoGrabM9Toggle:SetValue(cfg.autograbm9) end
	if uiElements.WhitelistEnabledToggle then uiElements.WhitelistEnabledToggle:SetValue(whitelistEnabled) end
	if uiElements.WhitelistModeDropdown then uiElements.WhitelistModeDropdown:SetValue(whitelistMode) end
	syncDistanceHitchanceEditor()
	updateEsp()
	updateC4Esp()
	refreshWhitelistPlayerList()
end
...
uiElements.DistanceBasedHitChanceToggle = sections.distanceHitchanceLeft:Toggle({
	...
	Callback = function(state)
		...
		if uiElements.AimMaxDistSlider then
			uiElements.AimMaxDistSlider:SetValue(cfg.aimmaxdist)
		end
	end,
	...
	Callback = function(value)
		if cfg.distancebasedhitchance or distanceHitchanceForcesAimMaxDistance then
			...
			if uiElements.AimMaxDistSlider and value ~= 0 then
				uiElements.AimMaxDistSlider:SetValue(0)
			end
			return
		end
...
-- ==================== WHITELIST UI (ENHANCED) ====================

sections.settingsLeft:Label({Text = "━━ Whitelist ━━"})

uiElements.WhitelistEnabledToggle = sections.settingsLeft:Toggle({
...
uiElements.WhitelistModeDropdown = sections.settingsLeft:Dropdown({
	Name = "Whitelist Mode",
	Default = whitelistMode,
	Options = {"userId", "username", "both"},
	Callback = function(value)
		whitelistMode = value
		saveWhitelist()
	end,
}, "WhitelistModeDropdown")

-- Player List Dropdown
uiElements.WhitelistPlayerDropdown = sections.settingsLeft:Dropdown({
	Name = "Server Players",
	Default = "Select a player...",
	Options = getPlayerListOptions(),
	Callback = function(value)
		-- Store selection for add/remove buttons
		uiElements._selectedWhitelistPlayer = value
	end,
}, "WhitelistPlayerDropdown")
...
sections.settingsLeft:Button({
	...
	Callback = function()
		...
		Window:Notify({
			Title = "Whitelist",
			Description = "Refreshed player list (" .. #whitelistPlayerList .. " players)",
			Lifetime = 3
		})
	end,
})
...
sections.settingsLeft:Button({
	...
	Callback = function()
		if not uiElements._selectedWhitelistPlayer or uiElements._selectedWhitelistPlayer == "Select a player..." then
			Window:Notify({
				Title = "Whitelist",
				Description = "Select a player from the dropdown first!",
				Lifetime = 3
			})
			return
		end
		addSelectedPlayerToWhitelist(uiElements._selectedWhitelistPlayer)
	end,
})
...
sections.settingsLeft:Button({
	...
	Callback = function()
		...
		removeSelectedPlayerFromWhitelist(uiElements._selectedWhitelistPlayer)
	end,
})

sections.settingsLeft:Label({Text = "━━ Manual Entry ━━"})

local whitelistInputText = ""

uiElements.WhitelistInput = sections.settingsLeft:Input({
	Name = "Username or UserId",
	Placeholder = "Enter username or UserId...",
	Callback = function(text)
		whitelistInputText = text
	end,
}, "WhitelistInput")
...
sections.settingsLeft:Button({
	...
	Callback = function()
		if whitelistInputText == "" then
			Window:Notify({
				Title = "Whitelist",
				Description = "Enter a username or UserId first!",
				Lifetime = 3
			})
			return
		end
		local mode, key = addToWhitelist(whitelistInputText)
		if mode then
			saveWhitelist()
			Window:Notify({
				Title = "Whitelist",
				Description = "Added " .. whitelistInputText .. " (" .. mode .. ")",
				Lifetime = 3
			})
			whitelistInputText = ""
			if uiElements.WhitelistInput then
				uiElements.WhitelistInput:SetValue("")
			end
		else
			Window:Notify({
				Title = "Whitelist",
...
sections.settingsLeft:Button({
	...
	Callback = function()
		...
		if removeFromWhitelist(whitelistInputText) then
			saveWhitelist()
			Window:Notify({
				Title = "Whitelist",
				Description = "Removed " .. whitelistInputText,
				Lifetime = 3
			})
		else
			Window:Notify({
...
sections.settingsLeft:Button({
	Name = "List Whitelisted",
	Callback = function()
		local list = getWhitelistList()
		if #list == 0 then
			Window:Notify({
				Title = "Whitelist",
				Description = "No players whitelisted!",
				Lifetime = 3
			})
		else
			Window:Notify({
...
sections.settingsLeft:Button({
	...
	Callback = function()
		...
		Window:Notify({
			Title = "Whitelist",
			Description = "Cleared all whitelisted players!",
			Lifetime = 3
		})
	end,
})
...
sections.settingsLeft:Button({
	...
	Callback = function()
		...
		local info = {name = target.Name, displayName = target.DisplayName}
		addToWhitelist(tostring(target.UserId), info)
		addToWhitelist(target.Name, info)
		saveWhitelist()
		refreshWhitelistPlayerList()
		Window:Notify({
			Title = "Whitelist",
			Description = "Added " .. target.Name .. " (ID: " .. target.UserId .. ")",
			Lifetime = 3
		})
	end,
})
...
refreshUI()
	...
	-- Aimbot Left
	uiElements.SilentAimToggle = sections.aimbotLeft:Toggle({Name = "Silent Aim", Default = cfg.enabled, Callback = function(s) cfg.enabled = s end}, "SilentAimToggle")
	uiElements.TeamCheckToggle = sections.aimbotLeft:Toggle({Name = "Team Check", Default = cfg.teamcheck, Callback = function(s) cfg.teamcheck = s end}, "TeamCheckToggle")
	uiElements.WallCheckToggle = sections.aimbotLeft:Toggle({Name = "Wall Check", Default = cfg.wallcheck, Callback = function(s) cfg.wallcheck = s end}, "WallCheckToggle")
	uiElements.DeathCheckToggle = sections.aimbotLeft:Toggle({Name = "Death Check", Default = cfg.deathcheck, Callback = function(s) cfg.deathcheck = s end}, "DeathCheckToggle")
	uiElements.VehicleCheckToggle = sections.aimbotLeft:Toggle({Name = "Vehicle Check", Default = cfg.vehiclecheck, Callback = function(s) cfg.vehiclecheck = s end}, "VehicleCheckToggle")
	uiElements.HostileCheckToggle = sections.aimbotLeft:Toggle({Name = "Hostile Check", Default = cfg.hostilecheck, Callback = function(s) cfg.hostilecheck = s end}, "HostileCheckToggle")
	uiElements.TrespassCheckToggle = sections.aimbotLeft:Toggle({Name = "Trespass Check", Default = cfg.trespasscheck, Callback = function(s) cfg.trespasscheck = s end}, "TrespassCheckToggle")
	uiElements.CriminalsSkipInmatesToggle = sections.aimbotLeft:Toggle({Name = "Criminals Skip Inmates", Default = cfg.criminalsnoinnmates, Callback = function(s) cfg.criminalsnoinnmates = s end}, "CriminalsSkipInmatesToggle")
	uiElements.InmatesSkipCriminalsToggle = sections.aimbotLeft:Toggle({Name = "Inmates Skip Criminals", Default = cfg.inmatesnocriminals, Callback = function(s) cfg.inmatesnocriminals = s end}, "InmatesSkipCriminalsToggle")
	uiElements.FFCheckToggle = sections.aimbotLeft:Toggle({Name = "ForceField Check", Default = cfg.ffcheck, Callback = function(s) cfg.ffcheck = s end}, "FFCheckToggle")

	-- Aimbot Right
	uiElements.ShieldBreakerToggle = sections.aimbotRight:Toggle({Name = "Shield Breaker", Default = cfg.shieldbreaker, Callback = function(s) cfg.shieldbreaker = s end}, "ShieldBreakerToggle")
	uiElements.ShieldFrontAngleSlider = sections.aimbotRight:Slider({Name = "Shield Front Angle", Default = cfg.shieldfrontangle, Minimum = -1, Maximum = 1, Precision = 1, Callback = function(v) cfg.shieldfrontangle = v end}, "ShieldFrontAngleSlider")
	uiElements.ShieldRandomHeadToggle = sections.aimbotRight:Toggle({Name = "Shield Random Head", Default = cfg.shieldrandomhead, Callback = function(s) cfg.shieldrandomhead = s end}, "ShieldRandomHeadToggle")
	uiElements.ShieldHeadChanceSlider = sections.aimbotRight:Slider({Name = "Shield Head Chance", Default = cfg.shieldheadchance, Minimum = 0, Maximum = 100, Precision = 0, Callback = function(v) cfg.shieldheadchance = v end}, "ShieldHeadChanceSlider")
	uiElements.TaserBypassHostileToggle = sections.aimbotRight:Toggle({Name = "Taser Bypass Hostile", Default = cfg.taserbypasshostile, Callback = function(s) cfg.taserbypasshostile = s end}, "TaserBypassHostileToggle")
	uiElements.TaserBypassTrespassToggle = sections.aimbotRight:Toggle({Name = "Taser Bypass Trespass", Default = cfg.taserbypasstrespass, Callback = function(s) cfg.taserbypasstrespass = s end}, "TaserBypassTrespassToggle")
	uiElements.TaserAlwaysHitToggle = sections.aimbotRight:Toggle({Name = "Taser Always Hit", Default = cfg.taseralwayshit, Callback = function(s) cfg.taseralwayshit = s end}, "TaserAlwaysHitToggle")
	uiElements.HitIfPlayerStillToggle = sections.aimbotRight:Toggle({Name = "Hit If Player Still", Default = cfg.ifplayerstill, Callback = function(s) cfg.ifplayerstill = s end}, "HitIfPlayerStillToggle")
	uiElements.StillThresholdSlider = sections.aimbotRight:Slider({Name = "Still Threshold", Default = cfg.stillthreshold, Minimum = 0, Maximum = 5, Precision = 1, Callback = function(v) cfg.stillthreshold = v end}, "StillThresholdSlider")
	uiElements.HitChanceSlider = sections.aimbotRight:Slider({Name = "Hit Chance", Default = cfg.hitchance, Minimum = 0, Maximum = 100, Precision = 0, Callback = function(v) cfg.hitchance = v end}, "HitChanceSlider")
	uiElements.HitChanceAutoOnlyToggle = sections.aimbotRight:Toggle({Name = "Hit Chance Auto Only", Default = cfg.hitchanceAutoOnly, Callback = function(s) cfg.hitchanceAutoOnly = s end}, "HitChanceAutoOnlyToggle")
	uiElements.AimMaxDistSlider = sections.aimbotRight:Slider({Name = "Aim Max Distance (0 = Any)", Default = cfg.aimmaxdist, Minimum = 0, Maximum = 5000, Precision = 0, Callback = function(v)
		if cfg.distancebasedhitchance or state.distanceHitchanceForcesAimMaxDistance then
			state.storedAimMaxDistanceBeforeDistanceHitchance = v
			cfg.aimmaxdist = 0
			return
		end
		cfg.aimmaxdist = v
	end}, "AimMaxDistSlider")
	uiElements.MissSpreadSlider = sections.aimbotRight:Slider({Name = "Miss Spread", Default = cfg.missspread, Minimum = 0, Maximum = 20, Precision = 1, Callback = function(v) cfg.missspread = v end}, "MissSpreadSlider")
	uiElements.ShotgunNaturalSpreadToggle = sections.aimbotRight:Toggle({Name = "Shotgun Natural Spread", Default = cfg.shotgunnaturalspread, Callback = function(s) cfg.shotgunnaturalspread = s end}, "ShotgunNaturalSpreadToggle")
	uiElements.ShotgunGameHandledToggle = sections.aimbotRight:Toggle({Name = "Shotgun Game Handled", Default = cfg.shotgungamehandled, Callback = function(s) cfg.shotgungamehandled = s end}, "ShotgunGameHandledToggle")
	uiElements.PrioritizeClosestToggle = sections.aimbotRight:Toggle({Name = "Prioritize Closest", Default = cfg.prioritizeclosest, Callback = function(s) cfg.prioritizeclosest = s end}, "PrioritizeClosestToggle")
	uiElements.PrioritizeCriminalsToggle = sections.aimbotRight:Toggle({Name = "Prioritize Criminals", Default = cfg.prioritizecriminals, Callback = function(s) cfg.prioritizecriminals = s end}, "PrioritizeCriminalsToggle")
	uiElements.TargetStickinessToggle = sections.aimbotRight:Toggle({Name = "Target Stickiness", Default = cfg.targetstickiness, Callback = function(s) cfg.targetstickiness = s end}, "TargetStickinessToggle")
	uiElements.StickinessDurationSlider = sections.aimbotRight:Slider({Name = "Stickiness Duration", Default = cfg.targetstickinessduration, Minimum = 0.1, Maximum = 2, Precision = 1, Callback = function(v) cfg.targetstickinessduration = v end}, "StickinessDurationSlider")
	uiElements.TargetStickinessRandomToggle = sections.aimbotRight:Toggle({Name = "Target Stickiness Random", Default = cfg.targetstickinessrandom, Callback = function(s) cfg.targetstickinessrandom = s end}, "TargetStickinessRandomToggle")
	uiElements.TargetStickinessMinSlider = sections.aimbotRight:Slider({Name = "Target Stickiness Min", Default = cfg.targetstickinessmin, Minimum = 0.1, Maximum = 1, Precision = 1, Callback = function(v) cfg.targetstickinessmin = v end}, "TargetStickinessMinSlider")
	uiElements.TargetStickinessMaxSlider = sections.aimbotRight:Slider({Name = "Target Stickiness Max", Default = cfg.targetstickinessmax, Minimum = 0.1, Maximum = 1, Precision = 1, Callback = function(v) cfg.targetstickinessmax = v end}, "TargetStickinessMaxSlider")
	uiElements.FovSlider = sections.aimbotRight:Slider({Name = "FOV", Default = cfg.fov, Minimum = 10, Maximum = 500, Precision = 0, Callback = function(v) cfg.fov = v end}, "FovSlider")
	uiElements.ShowFovToggle = sections.aimbotRight:Toggle({Name = "Show FOV", Default = cfg.showfov, Callback = function(s) cfg.showfov = s end}, "ShowFovToggle")
	uiElements.StaticFovToggle = sections.aimbotRight:Toggle({Name = "Static FOV", Default = cfg.staticfov, Callback = function(s) cfg.staticfov = s end}, "StaticFovToggle")
	uiElements.ShowTargetLineToggle = sections.aimbotRight:Toggle({Name = "Show Target Line", Default = cfg.showtargetline, Callback = function(s) cfg.showtargetline = s end}, "ShowTargetLineToggle")
	uiElements.AimPartDropdown = sections.aimbotRight:Dropdown({Name = "Aim Part", Default = cfg.aimpart, Options = cfg.partslist, Callback = function(v) cfg.aimpart = v end}, "AimPartDropdown")
	uiElements.RandomPartsToggle = sections.aimbotRight:Toggle({Name = "Random Parts", Default = cfg.randomparts, Callback = function(s) cfg.randomparts = s end}, "RandomPartsToggle")

	-- Distance Hitchance
	uiElements.DistanceBasedHitChanceToggle = sections.distanceHitchanceLeft:Toggle({Name = "Distance Based Hitchance", Default = cfg.distancebasedhitchance, Callback = function(s)
		setDistanceHitchanceEnabledState(s)
		if uiElements.AimMaxDistSlider then uiElements.AimMaxDistSlider:SetValue(cfg.aimmaxdist) end
	end}, "DistanceBasedHitChanceToggle")

	uiElements.DistanceHitchancePointDropdown = sections.distanceHitchanceLeft:Dropdown({Name = "Selected Point", Default = "1. 200 studs -> 30%", Options = {"1. 200 studs -> 30%"}, Callback = function(v)
		if state.syncingDistanceHitchanceEditor then return end
		local selectedIndex = tonumber(tostring(v):match("^(%d+)%."))
		if selectedIndex then
			state.selectedDistanceHitchancePoint = selectedIndex
			syncDistanceHitchanceEditor()
		end
	end}, "DistanceHitchancePointDropdown")

	sections.distanceHitchanceLeft:Button({Name = "Add Point", Callback = function()
		local points = ensureEditableDistanceHitchancePoints()
		local lastPoint = points[#points]
		local newPoint = {distance = lastPoint and (lastPoint.distance + 100) or 200, chance = lastPoint and math.max(lastPoint.chance - 8, 0) or 30}
		table.insert(points, newPoint)
		table.sort(points, function(a, b) return a.distance < b.distance end)
		for index, point in ipairs(points) do
			if point == newPoint then state.selectedDistanceHitchancePoint = index break end
		end
		setDistanceHitchanceEnabledState(true)
		refreshUI()
	...
	sections.distanceHitchanceLeft:Button({Name = "Remove Selected", Callback = function()
		local points = ensureEditableDistanceHitchancePoints()
		if #points <= 1 then
			Window:Notify({Title = "Distance Hitchance", Description = "Keep at least one point in the list.", Lifetime = 4})
			return
		end
		table.remove(points, state.selectedDistanceHitchancePoint)
		state.selectedDistanceHitchancePoint = math.clamp(state.selectedDistanceHitchancePoint, 1, #points)
		refreshUI()
		Window:Notify({Title = "Distance Hitchance", Description = "Removed the selected point.", Lifetime = 4})
	end})

	sections.distanceHitchanceLeft:Button({Name = "Load Legit Preset", Callback = function()
		loadLegitDistanceHitchancePreset()
		refreshUI()
	...
	sections.distanceHitchanceLeft:Button({Name = "Load Blatant Preset", Callback = function()
		loadBlatantDistanceHitchancePreset()
		refreshUI()
	...
	sections.distanceHitchanceLeft:Button({Name = "Reset to Default", Callback = function()
		cfg.distancehitchancepoints = getDefaultDistanceHitchancePoints()
		state.selectedDistanceHitchancePoint = 1
		refreshUI()
		Window:Notify({Title = "Distance Hitchance", Description = "Reset the distance list back to the default build.", Lifetime = 4})
	end})

	uiElements.SelectedDistancePointDistanceSlider = sections.distanceHitchanceRight:Slider({Name = "Selected Distance", Default = 200, Minimum = 0, Maximum = 5000, Precision = 0, Callback = function(v)
		if state.syncingDistanceHitchanceEditor then return end
		local points = ensureEditableDistanceHitchancePoints()
		local point = points[state.selectedDistanceHitchancePoint]
		if not point then return end
		point.distance = v
		table.sort(points, function(a, b) return a.distance < b.distance end)
		for index, existingPoint in ipairs(points) do
			if existingPoint == point then state.selectedDistanceHitchancePoint = index break end
		end
		syncDistanceHitchanceEditor()
	end}, "SelectedDistancePointDistanceSlider")

	uiElements.SelectedDistancePointHitchanceSlider = sections.distanceHitchanceRight:Slider({Name = "Selected Hitchance", Default = 30, Minimum = 0, Maximum = 100, Precision = 0, Callback = function(v)
		if state.syncingDistanceHitchanceEditor then return end
		local points = ensureEditableDistanceHitchancePoints()
		local point = points[state.selectedDistanceHitchancePoint]
		if not point then return end
		point.chance = v
		syncDistanceHitchanceEditor()
	end}, "SelectedDistancePointHitchanceSlider")

	-- ESP Left
	uiElements.EspToggle = sections.espLeft:Toggle({Name = "ESP", Default = cfg.esp, Callback = function(s) cfg.esp = s updateEsp() end}, "EspToggle")
	uiElements.EspTeamCheckToggle = sections.espLeft:Toggle({Name = "ESP Team Check", Default = cfg.espteamcheck, Callback = function(s) cfg.espteamcheck = s updateEsp() end}, "EspTeamCheckToggle")
	uiElements.ShowTeamToggle = sections.espLeft:Toggle({Name = "Show Team", Default = cfg.espshowteam, Callback = function(s) cfg.espshowteam = s updateEsp() end}, "ShowTeamToggle")
	uiElements.EspMaxDistanceSlider = sections.espLeft:Slider({Name = "ESP Max Distance (0 = Any)", Default = cfg.espmaxdist, Minimum = 0, Maximum = 5000, Precision = 0, Callback = function(v) cfg.espmaxdist = v updateEsp() end}, "EspMaxDistanceSlider")
	uiElements.ShowDistanceToggle = sections.espLeft:Toggle({Name = "Show Distance", Default = cfg.espshowdist, Callback = function(s) cfg.espshowdist = s updateEsp() end}, "ShowDistanceToggle")
	uiElements.UseTeamColorsToggle = sections.espLeft:Toggle({Name = "Use Team Colors", Default = cfg.espuseteamcolors, Callback = function(s) cfg.espuseteamcolors = s updateEsp() end}, "UseTeamColorsToggle")
	uiElements.EspColorPicker = sections.espLeft:Colorpicker({Name = "ESP Color", Default = cfg.espcolor, Callback = function(c) cfg.espcolor = c updateEsp() end}, "EspColorPicker")
	uiElements.GuardsColorPicker = sections.espLeft:Colorpicker({Name = "Guards Color", Default = cfg.espguards, Callback = function(c) cfg.espguards = c updateEsp() end}, "GuardsColorPicker")
	uiElements.InmatesColorPicker = sections.espLeft:Colorpicker({Name = "Inmates Color", Default = cfg.espinmates, Callback = function(c) cfg.espinmates = c updateEsp() end}, "InmatesColorPicker")
	uiElements.CriminalsColorPicker = sections.espLeft:Colorpicker({Name = "Criminals Color", Default = cfg.espcriminals, Callback = function(c) cfg.espcriminals = c updateEsp() end}, "CriminalsColorPicker")
	uiElements.TeamColorPicker = sections.espLeft:Colorpicker({Name = "Team Color", Default = cfg.espteam, Callback = function(c) cfg.espteam = c updateEsp() end}, "TeamColorPicker")

	-- ESP Right
	uiElements.C4EspToggle = sections.espRight:Toggle({Name = "C4 ESP", Default = cfg.c4esp, Callback = function(s) cfg.c4esp = s updateC4Esp() end}, "C4EspToggle")
	uiElements.C4EspMaxDistanceSlider = sections.espRight:Slider({Name = "C4 ESP Max Distance (0 = Any)", Default = cfg.c4espmaxdist, Minimum = 0, Maximum = 5000, Precision = 0, Callback = function(v) cfg.c4espmaxdist = v updateC4Esp() end}, "C4EspMaxDistanceSlider")
	uiElements.C4ShowDistanceToggle = sections.espRight:Toggle({Name = "C4 Show Distance", Default = cfg.c4espshowdist, Callback = function(s) cfg.c4espshowdist = s updateC4Esp() end}, "C4ShowDistanceToggle")
	uiElements.C4EspColorPicker = sections.espRight:Colorpicker({Name = "C4 ESP Color", Default = cfg.c4espcolor, Callback = function(c) cfg.c4espcolor = c updateC4Esp() end}, "C4EspColorPicker")

	-- Autoshoot
	uiElements.AutoshootToggle = sections.autoshootLeft:Toggle({Name = "Autoshoot", Default = cfg.autoshoot, Callback = function(s) cfg.autoshoot = s end}, "AutoshootToggle")
	uiElements.AutoshootWeaponDropdown = sections.autoshootRight:Dropdown({Name = "Autoshoot Weapon", Default = cfg.autoshootweapon, Options = {"Any", "Taser", "M9", "AK-47", "M4A1", "Remington 870", "Revolver", "Shotgun", "Sniper", "Automatic"}, Callback = function(v) cfg.autoshootweapon = v end}, "AutoshootWeaponDropdown")
	uiElements.AutoshootDelaySlider = sections.autoshootLeft:Slider({Name = "Autoshoot Delay", Default = cfg.autoshootdelay, Minimum = 0.05, Maximum = 0.5, Precision = 1, Callback = function(v) cfg.autoshootdelay = v end}, "AutoshootDelaySlider")
	uiElements.AutoshootStartDelaySlider = sections.autoshootLeft:Slider({Name = "Autoshoot Start Delay", Default = cfg.autoshootstartdelay, Minimum = 0, Maximum = 1, Precision = 1, Callback = function(v) cfg.autoshootstartdelay = v end}, "AutoshootStartDelaySlider")
	uiElements.AutoGrabToggle = sections.autoshootRight:Toggle({Name = "Auto Grab", Default = cfg.autograb, Callback = function(s) cfg.autograb = s end}, "AutoGrabToggle")
	uiElements.AutoGrabDistanceSlider = sections.autoshootRight:Slider({Name = "Auto Grab Distance", Default = cfg.autograbdistance, Minimum = 0, Maximum = 12, Precision = 1, Callback = function(v) cfg.autograbdistance = v end}, "AutoGrabDistanceSlider")
	uiElements.AutoGrabDelaySlider = sections.autoshootRight:Slider({Name = "Auto Grab Delay", Default = cfg.autograbdelay, Minimum = 0, Maximum = 3, Precision = 1, Callback = function(v) cfg.autograbdelay = v end}, "AutoGrabDelaySlider")
	uiElements.AutoGrabKeycardToggle = sections.autoshootRight:Toggle({Name = "Grab Keycards", Default = cfg.autograbkeycard, Callback = function(s) cfg.autograbkeycard = s end}, "AutoGrabKeycardToggle")
	uiElements.AutoGrabM9Toggle = sections.autoshootRight:Toggle({Name = "Grab M9", Default = cfg.autograbm9, Callback = function(s) cfg.autograbm9 = s end}, "AutoGrabM9Toggle")
	-- ==================== CONFIG SYSTEM ====================
	...
	local function getAutoLoadConfigName()
		...
		if not isfolder(configFolder) or not isfile(state.autoloadConfigFile) then return "" end
		return tostring(readfile(state.autoloadConfigFile) or ""):gsub("^%s+", ""):gsub("%s+$", "")
	end

	local function setAutoLoadConfigName(name)
		if not isfolder or not writefile then return false end
		if not isfolder(configFolder) then makefolder(configFolder) end
		writefile(state.autoloadConfigFile, tostring(name or ""))
		return true
	end

	local function clearAutoLoadConfigName()
		if not isfolder or not isfile or not delfile then return false end
		if isfile(state.autoloadConfigFile) then delfile(state.autoloadConfigFile) end
		return true
	end

	state.currentAutoLoadConfigName = getAutoLoadConfigName()
	if state.currentAutoLoadConfigName ~= "" then
		if loadConfig(state.currentAutoLoadConfigName) then
			Window:Notify({Title = "Auto Load", Description = "Loaded config: " .. state.currentAutoLoadConfigName, Lifetime = 4})
		else
			Window:Notify({Title = "Auto Load", Description = "Couldn't load config: " .. state.currentAutoLoadConfigName, Lifetime = 4})
		end
	end
	syncDistanceHitchanceAimMaxDistanceFromConfig()
...
-- ==================== REFRESH UI ====================
	local function refreshUI()
		local function safeSet(element, method, value)
			if element and element[method] then
				pcall(function() element[method](element, value) end)
			end
		end

		safeSet(uiElements.SilentAimToggle, "SetValue", cfg.enabled)
		safeSet(uiElements.TeamCheckToggle, "SetValue", cfg.teamcheck)
		safeSet(uiElements.WallCheckToggle, "SetValue", cfg.wallcheck)
		safeSet(uiElements.DeathCheckToggle, "SetValue", cfg.deathcheck)
		safeSet(uiElements.VehicleCheckToggle, "SetValue", cfg.vehiclecheck)
		safeSet(uiElements.HostileCheckToggle, "SetValue", cfg.hostilecheck)
		safeSet(uiElements.TrespassCheckToggle, "SetValue", cfg.trespasscheck)
		safeSet(uiElements.CriminalsSkipInmatesToggle, "SetValue", cfg.criminalsnoinnmates)
		safeSet(uiElements.InmatesSkipCriminalsToggle, "SetValue", cfg.inmatesnocriminals)
		safeSet(uiElements.FFCheckToggle, "SetValue", cfg.ffcheck)
		safeSet(uiElements.ShieldBreakerToggle, "SetValue", cfg.shieldbreaker)
		safeSet(uiElements.ShieldFrontAngleSlider, "SetValue", cfg.shieldfrontangle)
		safeSet(uiElements.ShieldRandomHeadToggle, "SetValue", cfg.shieldrandomhead)
		safeSet(uiElements.ShieldHeadChanceSlider, "SetValue", cfg.shieldheadchance)
		safeSet(uiElements.TaserBypassHostileToggle, "SetValue", cfg.taserbypasshostile)
		safeSet(uiElements.TaserBypassTrespassToggle, "SetValue", cfg.taserbypasstrespass)
		safeSet(uiElements.TaserAlwaysHitToggle, "SetValue", cfg.taseralwayshit)
		safeSet(uiElements.HitIfPlayerStillToggle, "SetValue", cfg.ifplayerstill)
		safeSet(uiElements.StillThresholdSlider, "SetValue", cfg.stillthreshold)
		safeSet(uiElements.HitChanceSlider, "SetValue", cfg.hitchance)
		safeSet(uiElements.HitChanceAutoOnlyToggle, "SetValue", cfg.hitchanceAutoOnly)
		safeSet(uiElements.DistanceBasedHitChanceToggle, "SetValue", cfg.distancebasedhitchance)
		safeSet(uiElements.AimMaxDistSlider, "SetValue", cfg.aimmaxdist)
		safeSet(uiElements.MissSpreadSlider, "SetValue", cfg.missspread)
		safeSet(uiElements.ShotgunNaturalSpreadToggle, "SetValue", cfg.shotgunnaturalspread)
		safeSet(uiElements.ShotgunGameHandledToggle, "SetValue", cfg.shotgungamehandled)
		safeSet(uiElements.PrioritizeClosestToggle, "SetValue", cfg.prioritizeclosest)
		safeSet(uiElements.PrioritizeCriminalsToggle, "SetValue", cfg.prioritizecriminals)
		safeSet(uiElements.TargetStickinessToggle, "SetValue", cfg.targetstickiness)
		safeSet(uiElements.StickinessDurationSlider, "SetValue", cfg.targetstickinessduration)
		safeSet(uiElements.TargetStickinessRandomToggle, "SetValue", cfg.targetstickinessrandom)
		safeSet(uiElements.TargetStickinessMinSlider, "SetValue", cfg.targetstickinessmin)
		safeSet(uiElements.TargetStickinessMaxSlider, "SetValue", cfg.targetstickinessmax)
		safeSet(uiElements.FovSlider, "SetValue", cfg.fov)
		safeSet(uiElements.ShowFovToggle, "SetValue", cfg.showfov)
		safeSet(uiElements.StaticFovToggle, "SetValue", cfg.staticfov)
		safeSet(uiElements.ShowTargetLineToggle, "SetValue", cfg.showtargetline)
		safeSet(uiElements.AimPartDropdown, "SetValue", cfg.aimpart)
		safeSet(uiElements.RandomPartsToggle, "SetValue", cfg.randomparts)
		safeSet(uiElements.EspToggle, "SetValue", cfg.esp)
		safeSet(uiElements.EspTeamCheckToggle, "SetValue", cfg.espteamcheck)
		safeSet(uiElements.ShowTeamToggle, "SetValue", cfg.espshowteam)
		safeSet(uiElements.EspMaxDistanceSlider, "SetValue", cfg.espmaxdist)
		safeSet(uiElements.ShowDistanceToggle, "SetValue", cfg.espshowdist)
		safeSet(uiElements.UseTeamColorsToggle, "SetValue", cfg.espuseteamcolors)
		safeSet(uiElements.EspColorPicker, "SetValue", cfg.espcolor)
		safeSet(uiElements.GuardsColorPicker, "SetValue", cfg.espguards)
		safeSet(uiElements.InmatesColorPicker, "SetValue", cfg.espinmates)
		safeSet(uiElements.CriminalsColorPicker, "SetValue", cfg.espcriminals)
		safeSet(uiElements.TeamColorPicker, "SetValue", cfg.espteam)
		safeSet(uiElements.C4EspToggle, "SetValue", cfg.c4esp)
		safeSet(uiElements.C4EspMaxDistanceSlider, "SetValue", cfg.c4espmaxdist)
		safeSet(uiElements.C4ShowDistanceToggle, "SetValue", cfg.c4espshowdist)
		safeSet(uiElements.C4EspColorPicker, "SetValue", cfg.c4espcolor)
		safeSet(uiElements.AutoshootToggle, "SetValue", cfg.autoshoot)
		safeSet(uiElements.AutoshootWeaponDropdown, "SetValue", cfg.autoshootweapon)
		safeSet(uiElements.AutoshootDelaySlider, "SetValue", cfg.autoshootdelay)
		safeSet(uiElements.AutoshootStartDelaySlider, "SetValue", cfg.autoshootstartdelay)
		safeSet(uiElements.AutoGrabToggle, "SetValue", cfg.autograb)
		safeSet(uiElements.AutoGrabDistanceSlider, "SetValue", cfg.autograbdistance)
		safeSet(uiElements.AutoGrabDelaySlider, "SetValue", cfg.autograbdelay)
		safeSet(uiElements.AutoGrabKeycardToggle, "SetValue", cfg.autograbkeycard)
		safeSet(uiElements.AutoGrabM9Toggle, "SetValue", cfg.autograbm9)
		safeSet(uiElements.WhitelistEnabledToggle, "SetValue", whitelistEnabled)
		safeSet(uiElements.WhitelistModeDropdown, "SetValue", whitelistMode)
		syncDistanceHitchanceEditor()
		updateEsp()
		updateC4Esp()
		refreshWhitelistPlayerList()
	...
	sections.settingsLeft:Input({Name = "Config Name", Placeholder = "Enter config name...", Callback = function(text) state.configNameInput = text end}, "ConfigNameInput")

	sections.settingsLeft:Button({Name = "Save Config", Callback = function()
		if state.configNameInput == "" then Window:Notify({Title = "Config", Description = "Please enter a config name!", Lifetime = 3}) return end
		if saveConfig(state.configNameInput) then Window:Notify({Title = "Config", Description = "Saved config: " .. state.configNameInput, Lifetime = 3})
		else Window:Notify({Title = "Config", Description = "Failed to save config!", Lifetime = 3}) end
	end})

	sections.settingsLeft:Button({Name = "Load Config", Callback = function()
		if state.configNameInput == "" then Window:Notify({Title = "Config", Description = "Please enter a config name!", Lifetime = 3}) return end
		if loadConfig(state.configNameInput) then refreshUI() Window:Notify({Title = "Config", Description = "Loaded config: " .. state.configNameInput, Lifetime = 3})
		else Window:Notify({Title = "Config", Description = "Config not found: " .. state.configNameInput, Lifetime = 3}) end
	end})

	sections.settingsLeft:Button({Name = "Delete Config", Callback = function()
		if state.configNameInput == "" then Window:Notify({Title = "Config", Description = "Please enter a config name!", Lifetime = 3}) return end
		if deleteConfig(state.configNameInput) then Window:Notify({Title = "Config", Description = "Deleted config: " .. state.configNameInput, Lifetime = 3})
		else Window:Notify({Title = "Config", Description = "Config not found: " .. state.configNameInput, Lifetime = 3}) end
	end})

	sections.settingsLeft:Button({Name = "List Configs", Callback = function()
		local configs = getConfigList()
		if #configs == 0 then Window:Notify({Title = "Configs", Description = "No configs found!", Lifetime = 5})
		else Window:Notify({Title = "Configs", Description = table.concat(configs, ", "), Lifetime = 10}) end
	end})

	sections.settingsRight:Button({Name = "Set Auto Load", Callback = function()
		if state.configNameInput == "" then Window:Notify({Title = "Auto Load", Description = "Enter a config name first.", Lifetime = 3}) return end
		if not isfile or not isfile(configFolder .. "/" .. state.configNameInput .. ".json") then Window:Notify({Title = "Auto Load", Description = "Config not found: " .. state.configNameInput, Lifetime = 3}) return end
		if setAutoLoadConfigName(state.configNameInput) then state.currentAutoLoadConfigName = state.configNameInput Window:Notify({Title = "Auto Load", Description = "Will auto load: " .. state.configNameInput, Lifetime = 4})
		else Window:Notify({Title = "Auto Load", Description = "Failed to save auto load config.", Lifetime = 3}) end
	end})

	sections.settingsRight:Button({Name = "Load Auto Config Now", Callback = function()
		local autoName = getAutoLoadConfigName()
		if autoName == "" then Window:Notify({Title = "Auto Load", Description = "No auto load config set.", Lifetime = 3}) return end
		if loadConfig(autoName) then state.currentAutoLoadConfigName = autoName state.configNameInput = autoName refreshUI() Window:Notify({Title = "Auto Load", Description = "Loaded auto config: " .. autoName, Lifetime = 4})
		else Window:Notify({Title = "Auto Load", Description = "Config not found: " .. autoName, Lifetime = 3}) end
	end})

	sections.settingsRight:Button({Name = "Clear Auto Load", Callback = function()
		if clearAutoLoadConfigName() then state.currentAutoLoadConfigName = "" Window:Notify({Title = "Auto Load", Description = "Cleared auto load config.", Lifetime = 3})
		else Window:Notify({Title = "Auto Load", Description = "Failed to clear auto load config.", Lifetime = 3}) end
	end})

	sections.settingsRight:Button({Name = "Reset to Defaults", Callback = function()
		for key, value in pairs(defaultCfg) do cfg[key] = deepCopyTable(value) end
		syncDistanceHitchanceAimMaxDistanceFromConfig()
		resetAimState()
		refreshUI()
	...
	uiElements.WhitelistEnabledToggle = sections.settingsLeft:Toggle({Name = "Whitelist Enabled", Default = whitelistEnabled, Callback = function(s) whitelistEnabled = s saveWhitelist() end}, "WhitelistEnabledToggle")
	uiElements.WhitelistModeDropdown = sections.settingsLeft:Dropdown({Name = "Whitelist Mode", Default = whitelistMode, Options = {"userId", "username", "both"}, Callback = function(v) whitelistMode = v saveWhitelist() end}, "WhitelistModeDropdown")

	uiElements.WhitelistPlayerDropdown = sections.settingsLeft:Dropdown({Name = "Server Players", Default = "Select a player...", Options = getPlayerListOptions(), Callback = function(v) _selectedWhitelistPlayer = v end}, "WhitelistPlayerDropdown")

	sections.settingsLeft:Button({Name = "Refresh Player List", Callback = function()
		refreshWhitelistPlayerList()
		Window:Notify({Title = "Whitelist", Description = "Refreshed player list (" .. #whitelistPlayerList .. " players)", Lifetime = 3})
	end})

	sections.settingsLeft:Button({Name = "Add Selected Player", Callback = function()
		if not _selectedWhitelistPlayer or _selectedWhitelistPlayer == "Select a player..." then Window:Notify({Title = "Whitelist", Description = "Select a player from the dropdown first!", Lifetime = 3}) return end
		addSelectedPlayerToWhitelist(_selectedWhitelistPlayer)
	end})

	sections.settingsLeft:Button({Name = "Remove Selected Player", Callback = function()
		if not _selectedWhitelistPlayer or _selectedWhitelistPlayer == "Select a player..." then Window:Notify({Title = "Whitelist", Description = "Select a player from the dropdown first!", Lifetime = 3}) return end
		removeSelectedPlayerFromWhitelist(_selectedWhitelistPlayer)
	end})

	sections.settingsLeft:Label({Text = "━━ Manual Entry ━━"})

	local whitelistInputText = ""
	uiElements.WhitelistInput = sections.settingsLeft:Input({Name = "Username or UserId", Placeholder = "Enter username or UserId...", Callback = function(text) whitelistInputText = text end}, "WhitelistInput")

	sections.settingsLeft:Button({Name = "Add Manual Entry", Callback = function()
		if whitelistInputText == "" then Window:Notify({Title = "Whitelist", Description = "Enter a username or UserId first!", Lifetime = 3}) return end
		local mode, key = addToWhitelist(whitelistInputText)
		if mode then
			saveWhitelist()
			Window:Notify({Title = "Whitelist", Description = "Added " .. whitelistInputText .. " (" .. mode .. ")", Lifetime = 3})
			whitelistInputText = ""
			if uiElements.WhitelistInput then uiElements.WhitelistInput:SetValue("") end
		else Window:Notify({Title = "Whitelist", Description = "Invalid input!", Lifetime = 3}) end
	end})

	sections.settingsLeft:Button({Name = "Remove Manual Entry", Callback = function()
		if whitelistInputText == "" then Window:Notify({Title = "Whitelist", Description = "Enter a username or UserId first!", Lifetime = 3}) return end
		if removeFromWhitelist(whitelistInputText) then
			saveWhitelist()
			Window:Notify({Title = "Whitelist", Description = "Removed " .. whitelistInputText, Lifetime = 3})
		else Window:Notify({Title = "Whitelist", Description = "Not found in whitelist!", Lifetime = 3}) end
	end})

	sections.settingsLeft:Button({Name = "List Whitelisted", Callback = function()
		local list = getWhitelistList()
		if #list == 0 then Window:Notify({Title = "Whitelist", Description = "No players whitelisted!", Lifetime = 3})
		else Window:Notify({Title = "Whitelist (" .. #list .. ")", Description = table.concat(list, ", "), Lifetime = 10}) end
	end})

	sections.settingsLeft:Button({Name = "Clear Whitelist", Callback = function()
		clearWhitelist()
		saveWhitelist()
		refreshWhitelistPlayerList()
	...
	sections.settingsLeft:Button({Name = "Add Current Target", Callback = function()
		local target = state.currentTarget
		if not target then Window:Notify({Title = "Whitelist", Description = "No current target!", Lifetime = 3}) return end
		local info = {name = target.Name, displayName = target.DisplayName}
		addToWhitelist(tostring(target.UserId), info)
		addToWhitelist(target.Name, info)
		saveWhitelist()
		refreshWhitelistPlayerList()
		Window:Notify({Title = "Whitelist", Description = "Added " .. target.Name .. " (ID: " .. target.UserId .. ")", Lifetime = 3})
	end})
