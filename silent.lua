-- ==================== AUTO-PATCH: Window:Notify Safety Wrapper ====================
-- Add this ENTIRE block to the VERY TOP of your script (before everything else)
-- This automatically fixes all "attempt to call a nil value" errors from Window:Notify
-- without requiring you to change any existing code.

local _patchNotifyQueue = {}
local _patchWindowRef = nil
local _patchNotifyReady = false

-- Create a proxy "Window" table that captures all :Notify calls before the real UI loads
local _patchProxyWindow = setmetatable({}, {
    __index = function(self, key)
        if key == "Notify" then
            return function(_, args)
                if _patchWindowRef and _patchWindowRef.Notify then
                    -- Real UI is ready, forward the call
                    return _patchWindowRef:Notify(args)
                else
                    -- UI not ready yet, queue the notification for later
                    table.insert(_patchNotifyQueue, args)
                    return nil
                end
            end
        end
        -- For any other key, return a dummy function that does nothing
        return function() end
    end
})

-- Replace the global "Window" reference with our proxy
-- This must run BEFORE any UI code executes
local _patchOldWindow = nil
if typeof(Window) ~= "nil" then
    _patchOldWindow = Window
end
Window = _patchProxyWindow

-- Function to flush queued notifications once real UI is ready
local function _patchFlushQueue()
    if not _patchWindowRef then return end
    for _, args in ipairs(_patchNotifyQueue) do
        pcall(function()
            _patchWindowRef:Notify(args)
        end)
    end
    _patchNotifyQueue = {}
    _patchNotifyReady = true
end

-- Hook into createUI to capture the real Window object
local _patchOldCreateUI = createUI
if _patchOldCreateUI then
    createUI = function(...)
        local result = _patchOldCreateUI(...)
        if result then
            _patchWindowRef = result
            _patchFlushQueue()
        end
        return result
    end
...
-- Also hook the return value of createUI if it's already defined later in the script
-- This runs after the script loads fully
local _patchSetupComplete = false
task.defer(function()
    if _patchSetupComplete then return end
    _patchSetupComplete = true
    
    -- Wait a bit for the script to initialize
    task.wait(0.1)
    
    -- If createUI exists and we haven't hooked it yet
    if typeof(createUI) == "function" and not _patchWindowRef then
        local old = createUI
        createUI = function(...)
            local result = old(...)
            if result then
    ...
    -- If Window was already set by the script's createUI, capture it
    if typeof(Window) == "table" and Window.Notify and Window ~= _patchProxyWindow then
        _patchWindowRef = Window
        _patchFlushQueue()
    end
end)
...
local function loadWhitelist()
    ...
    for _, entry in ipairs(data.list or {}) do
        local key = entry.key
        if key then  -- ADD THIS CHECK
            local playerInfo = entry.name and {name = entry.name, displayName = entry.displayName} or nil
            local id = tonumber(key)
            if id then 
                whitelist[id] = playerInfo or true
            else 
                whitelist[tostring(key):lower()] = playerInfo or true 
            end
        end
    end
end

local notify = notify or function(title, text, duration)
    pcall(function()
...
local state = {
    ...
    lastAutoTarget = nil,
    lastReloadRequest = 0,
    playerSettings = ReplicatedStorage:FindFirstChild("PlayerSettings"),
    mobileCursorOffset = 0,
    isInsideDynThumbFrame = nil,
    giverPressedRemote = ReplicatedStorage:FindFirstChild("Remotes") and ReplicatedStorage.Remotes:FindFirstChild("GiverPressed"),
    trackedGrabbables = {},
    sharedModules = ReplicatedStorage:FindFirstChild("SharedModules"),
    dynThumbModule = sharedModules and sharedModules:FindFirstChild("isInsideDynThumbFrame"),
    firstSeenGrabbables = {},
    lastAutoGrab = 0
}
...
do
	if state.dynThumbModule then
		local ok, result = pcall(require, state.dynThumbModule)
		if ok and typeof(result) == "function" then
			state.isInsideDynThumbFrame = result
		end
	end
end
...
local function trackGrabbable(obj)
	if isSupportedGrabbable(obj) then
		state.trackedGrabbables[obj] = true
	end
end

local function untrackGrabbable(obj)
	state.trackedGrabbables[obj] = nil
	state.firstSeenGrabbables[obj] = nil
end

local function updateAutoGrab(now)
	if not cfg.autograb or not state.giverPressedRemote then
		return
	end
	if now - state.lastAutoGrab < 0.05 then
		return
	end
	...
	for item in pairs(state.trackedGrabbables) do
		if not item or not item.Parent then
			untrackGrabbable(item)
		elseif not shouldAutoGrabItem(item) then
			state.firstSeenGrabbables[item] = nil
		elseif isOwnedGrabbable(item) then
			...
			if part and distSq(root.Position, part.Position) <= grabDistanceSq then
				if not state.firstSeenGrabbables[item] then
					state.firstSeenGrabbables[item] = now
				elseif now - state.firstSeenGrabbables[item] >= requiredDelay then
					state.lastAutoGrab = now
					state.firstSeenGrabbables[item] = nil
					pcall(state.giverPressedRemote.FireServer, state.giverPressedRemote, item)
					return
				end
			else
				state.firstSeenGrabbables[item] = nil
...
local function createUI()
    if state.uiCreated then return state.Window end  -- Prevent double creation
